...
//   const API_BASE = "http://127.0.0.1:8001/api/tasks";
...
//   useEffect(() => { if (!isLoading) fetchTasks(); }, [user, isLoading]);
...
//               <p className="text-[10px] text-amber-400 font-black tracking-[0.3em] uppercase italic">
//                 {activeTasks.length} Operations Pending
//               </p>
//             </div>
//           </div>
//         </header>
...
//                     className="w-full bg-transparent border-none text-xl font-bold focus:outline-none text-white placeholder:text-slate-800"
//                   />
//                 </div>
...
//                     className="w-full bg-transparent border-none text-sm text-slate-500 focus:outline-none placeholder:text-slate-800"
//                   />
//                   <button type="submit" className="bg-white text-black text-[10px] font-black px-6 py-2 rounded-full hover:bg-purple-500 hover:text-white transition-all uppercase tracking-widest active:scale-95">
//                     Deploy
//                   </button>
//                 </div>
//               </div>
...
//                     <div key={task.id} onClick={() => setSelectedTask(task)} className="group flex items-center justify-between p-5 rounded-xl bg-[#111114]/50 border border-white/[0.03] hover:border-purple-500/20 hover:bg-[#16161a] transition-all cursor-pointer">
...
//                   <h2 className="text-[10px] font-black text-amber-500/40 uppercase tracking-[0.4em] mb-6 flex items-center gap-2">
//                     <Sparkles size={14} /> Accomplished Mission
//                   </h2>
...
//       {/* FIXED DRAWER - NO SCROLLBAR, SYMMETRICAL BUTTONS */}
//       {selectedTask && (
...
//               <span className="text-[10px] font-black uppercase tracking-[0.2em] text-amber-500 flex items-center gap-2">
//                 <AlertCircle size={14} /> Intelligence Brief
//               </span>
//               <button onClick={() => setSelectedTask(null)} className="p-2 hover:bg-white/5 rounded-full text-slate-500 hover:text-white transition-colors"><X size={18} /></button>
//             </div>f
...
//                   className="w-full bg-transparent text-xl font-black text-white focus:outline-none resize-none leading-tight"
//                   rows={2}
//                 />
//               </div>
...
//                  <button 
//                   onClick={() => updateTask(selectedTask.id, { ...selectedTask, completed: !selectedTask.completed })}
//                   className={`flex items-center justify-center gap-3 px-4 py-3 rounded-xl text-[10px] font-black transition-all border ${selectedTask.completed ? 'bg-purple-600 border-purple-400 text-white shadow-[0_0_20px_rgba(168,85,247,0.4)]' : 'bg-slate-900 border-white/5 text-amber-500'}`}
//                 >
//                   {selectedTask.completed ? <CheckCircle size={14} /> : <Circle size={14} />}
//                   {selectedTask.completed ? 'COMPLETED' : 'MARK AS DONE'}
//                 </button>
//               </div>

//               <div className="space-y-3">
//                 <div className="flex items-center justify-between">
//                   <label className="text-[9px] font-black text-slate-700 uppercase tracking-widest">Briefing Detail</label>
...
//                   placeholder="No additional intel..."
//                   className="w-full bg-[#080809] border border-white/5 rounded-xl p-4 text-[13px] text-slate-400 focus:outline-none focus:border-purple-500/30 h-[120px] resize-none leading-relaxed"
//                 />
//               </div>
//             </div>
...
//                 className="flex-1 border border-white/10 text-slate-400 hover:text-red-500 hover:border-red-500/50 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 flex items-center justify-center gap-2"
//               >
//                 <Trash2 size={14} /> Delete
//               </button>
//               <button 
//                 onClick={() => setSelectedTask(null)} 
//                 className="flex-1 bg-white text-black py-3 rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl transition-all active:scale-95 flex items-center justify-center"
//               >
//                 Dismiss
//               </button>
//             </div>
...
export default function ProfessionalTasks() {
  ...
  // --- LOGIC FOR ARCHIVE SYNC ---
  const syncWithArchive = (task, isCompleted) => {
    const existingArchive = JSON.parse(localStorage.getItem('missionArchive') || '[]');
    
    if (isCompleted) {
      // Add to archive if not already there
      if (!existingArchive.find(t => t.id === task.id)) {
        const archiveEntry = { ...task, completedAt: new Date().toLocaleDateString() };
        localStorage.setItem('missionArchive', JSON.stringify([...existingArchive, archiveEntry]));
      }
    } else {
      // Remove from archive if task is unmarked as done
      const filteredArchive = existingArchive.filter(t => t.id !== task.id);
  ...
  const deleteTask = async (id) => {
    const res = await fetch(`${API_BASE}/${id}`, { method: 'DELETE' });
    if (res.ok) { 
      // Archive se bhi delete kardo agar task hamesha k liye delete ho raha hai
      const existingArchive = JSON.parse(localStorage.getItem('missionArchive') || '[]');
      localStorage.setItem('missionArchive', JSON.stringify(existingArchive.filter(t => t.id !== id)));
